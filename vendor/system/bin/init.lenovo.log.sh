...
if [ $(getprop persist.sys.lenovo.log) = TRUE ]; then
	...
	echo "persist.sys.lenovo.log.system: $(getprop persist.sys.lenovo.log.system)" >> $LOGDISK"/log/"$FILENAME/lenovolog_property
        ...
        echo "persist.sys.lenovo.log.mass: $(getprop persist.sys.lenovo.log.mass)" >> $LOGDISK"/log/"$FILENAME/lenovolog_property
        if [ $(getprop persist.sys.lenovo.log.mass) = TRUE ]; then
            setprop ctl.start mainlog_mass
        else
            setprop ctl.stop mainlog_mass
        fi
	...
	echo "persist.sys.lenovo.log.kernel: $(getprop persist.sys.lenovo.log.kernel)" >> $LOGDISK"/log/"$FILENAME/lenovolog_property
        ...
        echo "persist.sys.lenovo.bugreport: $(getprop persist.sys.lenovo.bugreport)" >> $LOGDISK"/log/"$FILENAME/lenovolog_property
        #if [ $(getprop persist.sys.lenovo.bugreport) = TRUE ]; then
        #    setprop ctl.start bugrep_log
        #else
        #    setprop ctl.stop bugrep_log
        #fi
