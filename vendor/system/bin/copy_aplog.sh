...
QXDMLOGFOLDERNAME="QXDM_"${FILENAME}
mkdir /sdcard/log/${QXDMLOGFOLDERNAME}
if [ -d "/sdcard/QXDMlog" ]; then
	cd /sdcard/QXDMlog
	if [ -z "$(ls)" ]; then
		echo "The /sdcard/QXDMlog directory is empty" > /sdcard/log/${QXDMLOGFOLDERNAME}/QXDM_log_status
		cd /sdcard && rm -rf /sdcard/QXDMlog
	else
		for qxdmlogfile in $(ls)
		do
			gzip -1 -c ${qxdmlogfile} > /sdcard/log/${QXDMLOGFOLDERNAME}/${qxdmlogfile}.gz
			rm ${qxdmlogfile}
		done
		echo "gzip /sdcard/QXDMlog files successfully " > /sdcard/log/${QXDMLOGFOLDERNAME}/QXDM_log_status
		cd /sdcard && rm -rf /sdcard/QXDMlog
	fi
else
	echo "Can not find /sdcard/QXDMlog directory" > /sdcard/log/${QXDMLOGFOLDERNAME}/QXDM_log_status
fi
